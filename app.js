...
const SUPABASE_CONFIG = {
  url: 'https://bwhjnemvmtyhkwbcdgct.supabase.co',
  anonKey: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJ3aGpuZW12bXR5aGt3YmNkZ2N0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODk4NTk4MDgsImV4cCI6MjEwNTQzNTgwOH0.ChFPF8MDt5xOLp5zTcU0Btk84btpsC5ItVsviexjiIk'
};

const supabase = (typeof window !== 'undefined' && window.supabase && SUPABASE_CONFIG.url) 
...
async function loadInitialMetadata() {
  try {
    ...
    try {
      ...
      if (varsRes.ok && faultsRes.ok) {
        vars = await varsRes.json();
        faults = await faultsRes.json();
      } else {
        throw new Error('Fallback to Supabase');
      }
    } catch (e) {
      if (supabase) {
        const { data: vData } = await supabase.from('tep_variables').select('*').order('id');
        const { data: fData } = await supabase.from('tep_fault_types').select('*').order('id');
        if (vData) {
          vars = vData.map(v => ({ id: v.id, tag: v.tag, name: v.name, unit: v.unit, type: v.type, desc: v.description }));
        }
        if (fData) {
          fData.forEach(f => {
            faults[f.id] = { name: f.name, type: f.type, desc: f.description };
          });
        }
      }
    }
    STATE.variables = vars;
    STATE.faultTypes = faults;
...
async function loadDashboardForMode(mode) {
  try {
    ...
    try {
      ...
      if (supabase) {
        const { data: mData } = await supabase.from('tep_mode_stats').select('*').eq('mode', mode).single();
        if (mData) {
          info = {
            mode: mData.mode,
            totalSamples: mData.total_samples,
            timeSteps: mData.time_steps,
            numVariables: mData.num_variables,
            totalClasses: mData.total_classes,
            labelDistribution: mData.label_distribution,
            foldDistribution: mData.fold_distribution
          };
        }
      }
    }
...
async function loadPcaData() {
  try {
    ...
    try {
      ...
      if (supabase) {
        const { data: points } = await supabase.from('tep_pca_projections').select('*').eq('mode', STATE.currentMode);
        if (points) {
          data = {
            mode: STATE.currentMode,
            explainedVariance: [42.8, 23.4],
            points: points.map(p => ({
              x: p.x,
              y: p.y,
              sampleIndex: p.sample_index,
              label: p.label,
              faultName: STATE.faultTypes[p.label]?.name || `Falha ${p.label}`,
              type: STATE.faultTypes[p.label]?.type || 'Outro'
            }))
          };
        }
      }
